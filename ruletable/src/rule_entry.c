...
static void display_ac_entry(struct ac_entry *entry)
{
	...
	target = (struct ac_proto_match*)((void*)entry + entry->target_offset);
...
static struct ac_entry *generate_ac_entry(struct ac_rule_item *rule_item)
{
	...
	entry_size += sizeof(struct ac_entry);
	entry_size += flow_match->match_size;
	entry_size += proto_match->match_size;
	entry_size += sizeof(struct ac_target);
	...
	entry->proto_match_offset = sizeof(struct ac_entry) + flow_match->match_size;
	entry->target_offset = entry->proto_match_offset + proto_match->match_size;
	entry->next_offset = entry->target_offset + sizeof(struct ac_target);
	memcpy((void*)entry + sizeof(struct ac_entry), flow_match, flow_match->match_size);
	memcpy((void*)entry + entry->proto_match_offset, proto_match, proto_match->match_size);
	memcpy((void*)entry + entry->target_offset, target, sizeof(struct ac_target));
