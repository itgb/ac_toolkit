...
static const char ipset_key_map[CONTROL_IPSET_TYPE_MAX][AC_IPSET_MAXNAMELEN] = {
...
*/
static int id_list_init(unsigned int **list, unsigned int *number, const nx_json *js) 
{
	int ret = -1, idx = 0;
...
static int ac_rule_item_init(struct ac_rule_item *rule_item, const nx_json *js)
{
	...
	if (check_list_valid(js_elem, AC_RULE_ID_MAXNUM) == 0) {
		if (id_list_init(&rule_item->src_zone_ids, &rule_item->src_zone_num, js_elem) == -1) {
			goto fail;
		} 
	}
	...
	if (check_list_valid(js_elem, AC_RULE_ID_MAXNUM) == 0) {
		if (id_list_init(&rule_item->src_ipgrp_ids, &rule_item->src_ipgrp_num, js_elem) == -1) {
			goto fail;
		} 
	}
	...
	if (check_list_valid(js_elem, AC_RULE_ID_MAXNUM) == 0) {
		if (id_list_init(&rule_item->dst_zone_ids, &rule_item->dst_zone_num, js_elem) == -1) {
			goto fail;
		} 
	}
	...
	if (check_list_valid(js_elem, AC_RULE_ID_MAXNUM) == 0) {
		if (id_list_init(&rule_item->dst_ipgrp_ids, &rule_item->dst_ipgrp_num, js_elem) == -1) {
			goto fail;
		} 
	}
	...
	if (check_list_valid(js_elem, AC_RULE_ID_MAXNUM) == 0) {
		if (id_list_init(&rule_item->proto_ids, &rule_item->proto_num, js_elem) == -1) {
			goto fail;
		} 
	}
...
void display_ac_rule(struct ac_rule *rule) 
{
	int i = 0, j = 0;
	if (rule == NULL) {
		AC_ERROR("invalid parameter.");
...
void display_control_rule(struct ac_rule *rule) 
{
	return display_ac_rule(rule);
}
...
void display_audit_rule(struct ac_rule *rule)
{
	return display_ac_rule(rule);
}
...
static void display_ac_set(struct ac_set *set)
{
	int idx = 0;
	if (set == NULL) {
		AC_ERROR("invalid parameters");
	...
	AC_DEBUG("Total number of control set:%d %d\n", set->number, set->updated);
	AC_PRINT("---------------SETS START------------\n");
	for (idx = 0; idx < set->number; ++idx) {
		if ((set->updated & (1 << idx)) && set->ipsets[idx]) {
			AC_PRINT("%s:%s\n", ipset_key_map[idx], set->ipsets[idx]);
		}
	}
	AC_PRINT("---------------SETS END------------\n\n");
}
...
void display_control_set(struct ac_set *set)
{
	display_ac_set(set);
}
...
void display_audit_set(struct ac_set *set)
{
	display_ac_set(set);
}
